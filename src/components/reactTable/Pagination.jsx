...
const Pagination = () => {
	...
	const paginationRange = useMemo(() => {
		...
		if (!shouldShowLeftDots && shouldShowRightDots) {
			const leftItemCount = 2 + 2;
			return [...Array.from({ length: leftItemCount }, (_, i) => i + 1), '...', totalPageCount];
		}

		if (shouldShowLeftDots && !shouldShowRightDots) {
			const rightItemCount = 2 + 2;
			const rightRange = Array.from({ length: rightItemCount }, (_, i) => i + totalPageCount - rightItemCount + 1);
			return [1, '...', ...rightRange];
		}

		if (shouldShowLeftDots && shouldShowRightDots) {
			const middleRange = Array.from({ length: rightSiblingIndex - leftSiblingIndex + 1 }, (_, i) => i + leftSiblingIndex);
